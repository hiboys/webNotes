...
/**
 *
 * This class generates thumbnail of any URL with the help of WebThumb
 API created by Joshua Eichorn
 *
 * @package WebThumb
...
class WebThumb
{
	 ...
	 * request a thumbnail
	 *
	 * @param string $url if you want to make thumbnail of a single
	 URL, specifcy it here
	 * @param integer $width optional width of the thumbnail
	 * @param integer $height optional height of the thumbnail
	 * @return string JobStatus with id, estimated time, starting
	 time and url of the job
	 */
		...
		"")
	{
		...
		$_request = "<webthumb>
			<apikey>{$this->_api}</apikey>{$requests}
		</webthumb>";
	 ...
	 *
	 * @param string $job_id the job id returned by
	 requestThumbnail() method* @return array JobStatus with id, submissionTime,
		 browserHeight, browserWidth, download URL, status and completionTime
	 */
	 ...
	 * @param string $size it could be either of five types,
	 "small","medium","medium2","large" and "zip"
	 * @return binarry data
	 */
	...
	private function _executeCurlRequest($request){
		$_session = curl_init();
		curl_setopt($_session, CURLOPT_URL, $this->_request_uri);
		// set url to post to
		curl_setopt ($_session, CURLOPT_POST, 1);
		curl_setopt ($_session, CURLOPT_POSTFIELDS, $request);
		curl_setopt($_session, CURLOPT_HTTPHEADER, array( 'ContentType: application/xml'));
		curl_setopt($_session, CURLOPT_RETURNTRANSFER, true);
		$_response = curl_exec($_session);
		return $_response;
	}
	private function _executeCurlRequest2($request)
	{
		$_session = curl_init();
		curl_setopt($_session, CURLOPT_URL, $this->_request_uri);
		// set url to post to
		curl_setopt ($_session, CURLOPT_POST, 1);
		curl_setopt ($_session, CURLOPT_POSTFIELDS, $request);
		curl_setopt($_session, CURLOPT_HTTPHEADER, array( 'ContentType: application/jpg'));
		curl_setopt($_session, CURLOPT_RETURNTRANSFER, true);
		$_response = curl_exec($_session);
		return $_response;
	}
